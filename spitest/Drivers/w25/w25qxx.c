...
void W25qxx_wait(void)
{
  ...
  while (w25qxx.StatusRegister1 != 0);
...
void 	W25qxx_WritePage(uint8_t *pBuffer	,uint32_t Page_Address,uint32_t NumByteToWrite_up_to_PageSize)
{
  ...
  W25qxx_Spi(Page_Address&0xFF);
	HAL_SPI_Transmit(&_W25QXX_SPI,pBuffer,NumByteToWrite_up_to_PageSize,500);	
	HAL_GPIO_WritePin(_W25QXX_CS_GPIO,_W25QXX_CS_PIN,GPIO_PIN_SET);
  W25qxx_WaitForWriteEnd();
	#if (_W25QXX_DEBUG==1)
...
//###################################################################################################################
void 	W25qxx_WriteNoCheck(uint8_t *pBuffer	,uint32_t Page_Address,uint32_t NumByteToWrite_up_to_PageSize)
{
	uint16_t pageremain;
	
	pageremain = 256-Page_Address%256;
	if (NumByteToWrite_up_to_PageSize <=pageremain) pageremain = NumByteToWrite_up_to_PageSize;
  
	while (1)
	{
		W25qxx_WritePage(pBuffer,Page_Address,pageremain);
		if (NumByteToWrite_up_to_PageSize == pageremain) break;
		else
		{
			pBuffer += pageremain;
			Page_Address += pageremain;
			NumByteToWrite_up_to_PageSize -= pageremain;
			if (NumByteToWrite_up_to_PageSize > 256) pageremain = 256;
			else pageremain = NumByteToWrite_up_to_PageSize;
...
void 	W25qxx_WriteSector(uint8_t *pBuffer	,uint32_t Sector_Address,uint32_t OffsetInByte	,uint32_t NumByteToWrite_up_to_SectorSize)
{
	...
	do
	{		
		W25qxx_WritePage(pBuffer,StartPage,BytesToWrite);
		StartPage++;
...
//###################################################################################################################
uint8_t 	W25qxx_ReadByte(uint8_t *pBuffer,uint32_t Bytes_Address)
{
	while(w25qxx.Lock==1)
		W25qxx_Delay(1);
	w25qxx.Lock=1;
